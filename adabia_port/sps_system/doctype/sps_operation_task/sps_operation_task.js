...
function setup_tab_listener(frm) {
	// Create new listener
	$("[data-fieldname='pdf_view_tab']").on('click', function () {
		load_pdf_content(frm);
	});
	$("[data-fieldname='preview_tab']").on('click', function () {
        render_task_preview(frm);
    });
}

function render_task_preview(frm) {
    if (!frm.fields_dict.task_preview) return;
    ...
    let html = `
		<div class="preview-container" style="padding: 20px; border: 1px solid #d1d8dd; border-radius: 8px; background-color: #f8f9fa; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
			...
			<div class="preview-content" style="display: grid; grid-template-columns: 150px 1fr; gap: 15px; align-items: baseline;">
				...
				<div style="font-weight: bold; color: #555;">${__('Title')}:</div>
				<div style="color: #000;">${doc.title || '-'}</div>
				...
				<div style="color: #000;">${doc.task_type || '-'}</div>

				${doc.status === 'Closed' ? `
					<div style="font-weight: bold; color: #555;">${__('Patch Number')}:</div>
					<div style="color: #1a73e8; font-weight: 500;">#${doc.patch_num || '-'}</div>
				` : ''}
				...
				${doc.task_type === 'New Request' ? `
					<div style="font-weight: bold; color: #555;">${__('Scopes')}:</div>
					<div style="color: #1a73e8; font-weight: 500;">#${doc.scopes || '-'}</div>
					<div style="font-weight: bold; color: #555;">${__('Requirements')}:</div>
					<div style="color: #1a73e8; font-weight: 500;">#${doc.requirements || '-'}</div>
					<div style="font-weight: bold; color: #555;">${__('Description')}:</div>
					<div style="color: #1a73e8; font-weight: 500;">#${doc.description || '-'}</div>
				` : ''}
			</div>
		</div>
	`;

    frm.get_field('task_preview').$wrapper.html(html);
}

async function load_pdf_content(frm) {
	const wrapper = frm.get_field('cr_preview').$wrapper;
